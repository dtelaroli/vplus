...
public class GsonDeserialization implements Deserializer {
	...
	public Object[] deserialize(InputStream inputStream, ResourceMethod method) {
		...
		ConsumesType consumesType = method.getResource().getType().getAnnotation(ConsumesType.class);
		if(consumesType != null && types.length > 0) {
			Class<?> definedType = consumesType.value();
			types[0] = definedType;
		}
